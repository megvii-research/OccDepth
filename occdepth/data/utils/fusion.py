...
class TSDFVolume:
    ...
    # regard voxel as sphere, and project it onto image for getting pixels
    # assume fx==fy
    @staticmethod
    @njit(parallel=True)
    def cam2allpixs(cam_pts, intr, vox_size, pattern_id):
        """Convert camera coordinates to pixel coordinates."""
        pixel_partern = [
            [[0, 0]],
            [[0, 0], [0, -1], [-1, 0], [1, 0], [0, 1]],
            ...
            [[0, 0], [0, -2], [-1, -1], [1, -1], [-2, 0], [2, 0], [-1, 1], [0, 2]],
            [
                [0, 0],
                [0, -2],
                [-1, -1],
        ...
        ]  # dso residual pattern
        intr = intr.astype(np.float32)
        fx, fy = intr[0, 0], intr[1, 1]
        cx, cy = intr[0, 2], intr[1, 2]
        pattern = pixel_partern[pattern_id]
        n_reserve_points = len(pattern)
        pix = np.empty((cam_pts.shape[0], n_reserve_points, 2), dtype=np.int64)
        for i in prange(cam_pts.shape[0]):
            x_center = int(np.round((cam_pts[i, 0] * fx / cam_pts[i, 2]) + cx))
            y_center = int(np.round((cam_pts[i, 1] * fy / cam_pts[i, 2]) + cy))
            for idx_pattern in range(n_reserve_points):
                pix[i, idx_pattern, 0] = int(x_center + pattern[idx_pattern][0])
                pix[i, idx_pattern, 1] = int(y_center + pattern[idx_pattern][1])
        return pix
