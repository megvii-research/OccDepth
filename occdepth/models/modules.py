...
from occdepth.models.DDR import Bottleneck3D
...
class SegmentationHeadCascadeCLS(nn.Module):
    """
    3D Segmentation heads to retrieve semantic segmentation at each scale.
    Formed by Dim expansion, Conv3D, ASPP block, Conv3D.
    Taken from https://github.com/cv-rits/LMSCNet/blob/main/LMSCNet/models/LMSCNet.py#L7
    ...
    def __init__(self, inplanes, planes, nbr_classes, dilations_conv_list):
        ...
        occ_classes = 2
        self.conv_classes = nn.Conv3d(
            planes + occ_classes, nbr_classes, kernel_size=3, padding=1, stride=1
        )
    ...
    def forward(self, x_in):
        ...
        x_occ = self.occ_classes(x_in)
        x_occ_softmax = self.softmax(x_occ)

        x_in = torch.cat([x_in, x_occ_softmax], dim=1)
...
class SegmentationHeadOccludedCLS(nn.Module):
    """
    3D Segmentation heads to retrieve semantic segmentation at each scale.
    Formed by Dim expansion, Conv3D, ASPP block, Conv3D.
    Taken from https://github.com/cv-rits/LMSCNet/blob/main/LMSCNet/models/LMSCNet.py#L7
...
class Convblock3d(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, bn_momentum, stride=1):
