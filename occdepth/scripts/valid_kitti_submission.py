    ...
    try:
        ...
        with zipfile.ZipFile(FLAGS.zipfile) as zipfile:
            if FLAGS.task == "segmentation" or FLAGS.task == "panoptic":
                ...
                directories = [
                    folder.filename
                    for folder in zipfile.infolist()
                ...
                for sequence in range(11, 22):
                    sequence_directory = "sequences/{}".format(sequence)
                    velodyne_directory = os.path.join(
                        FLAGS.dataset, "sequences/{}/velodyne/".format(sequence)
                    )

                    velodyne_files = sorted(
                        [
                            os.path.join(velodyne_directory, file)
                            for file in os.listdir(velodyne_directory)
                        ]
                    )
                    label_files = sorted(
                        [
                            os.path.join(
                                sequence_directory,
                                "predictions",
                                os.path.splitext(filename)[0] + ".label",
                            )
                    ...
                    for velodyne_file, label_file in zip(velodyne_files, label_files):
                        ...
                        if label_file not in prediction_files:
                            raise ValidationException(
                                '"' + label_file + '" is missing inside zip.'
                            )

                        num_labels = (
                            prediction_files[label_file].file_size / uint32_bytes
                        )
                        if num_labels != num_points:
                            raise ValidationException(
                                'label file "'
                                + label_file
                                + "' should have {} labels, but found {} labels!".format(
                                    int(num_points), int(num_labels)
                                )
                            )
                ...
                prediction_files = {
                    str(info.filename): info
                    for info in zipfile.infolist()
                    if not info.filename.endswith("/")
                }

                # description.txt is optional and one should not get an error.
                if "description.txt" in prediction_files:
                    del prediction_files["description.txt"]
                ...
                for sequence in range(11, 22):
                    ...
                    sequence_directory = "sequences/{}".format(sequence)
                    voxel_directory = os.path.join(
                        FLAGS.dataset, "sequences/{}/voxels/".format(sequence)
                    )

                    voxel_files = sorted(
                        [
                            os.path.join(voxel_directory, file)
                            for file in os.listdir(voxel_directory)
                    ...
                    for voxel_file, label_file in zip(voxel_files, label_files):
                        ...
                        num_voxels = input_voxels.shape[
                            0
                        ]  # fixed volume (= 256 * 256 * 32)!
                        ...
                        num_labels = (
                            prediction_files[label_file].file_size / uint16_bytes
                        )  # expecting uint16 for labels.
